...
def count_reads_at_target(crams, outputdir, pos, user, group, samtools, hotcount, pear, log, output, query):
    ...
    for sample, cram in crams:
        ...
        subprocess.check_output(f"{samtools} view {cram} {pos} -F 12 -b -o {bam}", stderr=subprocess.STDOUT, shell=True)
...
pear_path = ""
msh2_hotspot_var = ""
vaf_detection_treshold = ""

### error messages ###
if not os.path.exist(results_dir):
...
for designs_plus_gene in designs_list:
    ...
    hotspot_count = {}
    query_list = f"{directory}{design}_{gene}.txt"
    ...
    if patients_design:
        ...
        MAPPING_files = {
            ...
            for cram in [os.path.join(input_dir, patient, f"{patient}-ready.cram"),
                         os.path.join(input_dir, patient, f"{patient}-ready.bam")]
        ...
        with open(output_txt, "w+") as txt:
            ...
            format_overview_title = workbook.add_format({'bold': 1, 'size': 18, 'color': 'black', 'align': 'left'})
            run_overview_format = workbook.add_format({'bold': 1, 'size': 14, 'color': 'black','bg_color': '#D8D8D8', 'align': 'centre'})
            header_overview_format = workbook.add_format({'bold': 1, 'size': 11, 'color': 'black', 'bg_color': '#D8D8D8', 'align': 'left'})
            body_overview_format = workbook.add_format({'bold': 0, 'size': 11, 'color': 'black', 'align': 'left'})
            body_overview_format_bg = workbook.add_format({'bold': 1, 'size': 11, 'color': 'black', 'bg_color': '#4db8ff', 'align': 'left'})
            ...
            for patient, my_tuple in sorted(hotspot_count.items(), key=lambda item: int(item[1][1]), reverse=True):
                ...
                my_row = f"{patient}\t{wt}"
                for colnum, alt in enumerate(alt_list, start=2):
                    ...
                    if frequency >= vaf_detection_treshold:
                        ...
                        worksheet.write(rownum, column, f"{frequency}")
